"""newsfile_gnews — Newsfile (newsfilecorp.com) discovery via Google News RSS.

Newsfile doesn't expose a public RSS feed for its global wire, and the /news
page is JS-rendered. So we use Google News RSS as a discovery layer for
mining-related Newsfile releases.

Unlike Accesswire (Cloudflare-blocked), Newsfile pages fetch cleanly — we can
extract full bodies. The key value of this firehose is *discovering* mining
tickers that aren't in tickers.json yet.

Mining classifier: strict — both headline keywords AND a Canadian ticker
extracted from the body before we ingest.
"""
...
# ---------- ticker extraction (same as thenewswire_wire) ----------
...
def gnews_query_url(days: int = 7) -> str:
    q = (
        "site:newsfilecorp.com (mining OR gold OR silver OR copper OR "
...
_TITLE_SUFFIX = re.compile(r"\s+-\s+(?:Newsfile|TMX\s+Newsfile)\s*$", re.I)
...
def _extract_body_html(real_url: str, client: httpx.Client) -> tuple[str, str]:
    """Fetch a Newsfile release page and extract title + body HTML."""
    ...
    for tag in soup(["script", "style", "nav", "header", "footer", "aside",
        ...
        tag.decompose()
    # Newsfile release body is typically inside .release-content or main article
    for sel in [
        "div.release-content", "div.release-body",
        "article", "main article", "main",
        "div.col-md-9", "div.content", "div#release-content",
    ]:
...
def list_mining_events(days: int = 7) -> Iterator[dict]:
    """Discover Newsfile mining items via GNews and yield event dicts.
    ...
    with httpx.Client(headers={"User-Agent": UA}, timeout=25) as client:
        for raw in items:
            ...
            yield {
                ...
                "source_name":  "newsfile",  # same source-name as per-company adapter
                "source_url":   real_url,
                "raw_headline": title,
                "raw_body":     body_text,
                "raw_html":     body_html,
...
# ---------- company name extraction (same approach as thenewswire) ----------
