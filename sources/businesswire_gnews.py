"""businesswire_gnews — Business Wire (businesswire.com) mining discovery via Google News RSS.

Same proven pattern as the other firehose adapters. Business Wire is mostly
US-focused but does carry some Canadian junior miner releases.

source_name="businesswire" — new source label (no per-company adapter exists yet).
"""
...
def gnews_query_url(days: int = 7) -> str:
    q = (
        "site:businesswire.com (mining OR gold OR silver OR copper OR "
...
_TITLE_SUFFIX = re.compile(r"\s+-\s+Business\s+Wire\s*$", re.I)
...
def _extract_body_html(real_url: str, client: httpx.Client) -> tuple[str, str]:
    """Body selector confirmed via probe: div.bw-release-body."""
    ...
    for sel in [
        "div.bw-release-body",
        "div.bw-release-content",
        "section.bw-release-body",
