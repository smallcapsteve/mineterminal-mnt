"""sync_globenewswire.py — pull GlobeNewswire mining releases via Google News, dedupe, ingest.

GlobeNewswire's native mining-specific RSS feed is unreliable (subjectcode/9
...
GlobeNewswire pages fetch cleanly so we get full bodies and extract Canadian
...
from sources import globenewswire_gnews as gng  # noqa: E402
...
def auto_onboard(ticker: str, name: str | None, exchange: str | None,
    ...
    new = {
        ...
        "source": "globenewswire_gnews",
        "source_params": {},
        "discovery_confidence": "globenewswire_auto",
        "discovered_at": today,
    }
    if exchange:
...
def main(days: int = 7) -> int:
    ...
    print(f"[sync_globenewswire] start {started}  days={days}")

    tickers = load_tickers()
    idx = known_tickers_index(tickers)
    print(f"[sync_globenewswire] known tickers: {len(idx)}")

    items = list(gng.list_mining_events(days=days))
    print(f"[sync_globenewswire] mining items from GNews: {len(items)}")
    ...
    for ev in items:
        ...
        if ticker not in idx:
            name = gng.extract_company_name(ev["raw_headline"], ev["raw_body"])
        ...
        ev_for_quality["_source_name"] = ev.get("source_name", "globenewswire")
    ...
    if onboarded:
        ...
        print(f"[sync_globenewswire] tickers.json saved ({len(tickers)} entries)")

    # 6. Backfill slugs for any new events from this source
    n_slug = 0
    for eid, hl in conn.execute(
        "SELECT event_id, raw_headline FROM events "
        "WHERE source_name='globenewswire' AND (slug IS NULL OR slug = '')"
    ):
    ...
    print(f"[sync_globenewswire] DONE  ingested={ingested}  fuzzy_dup_skip={dup_skip}  "
