"""sync_cnw.py — pull CNW mining releases via Google News, dedupe, ingest.

CNW's native mining-specific RSS feed is unreliable (subjectcode/9
...
CNW pages fetch cleanly so we get full bodies and extract Canadian
tickers directly. New mining tickers are auto-onboarded into tickers.json.

source_name="cnw" — same as the per-company adapter, so the
...
from sources import cnw_gnews as cng  # noqa: E402
...
def main(days: int = 7) -> int:
    ...
    print(f"[sync_cnw] start {started}  days={days}")

    tickers = load_tickers()
    idx = known_tickers_index(tickers)
    print(f"[sync_cnw] known tickers: {len(idx)}")

    items = list(cng.list_mining_events(days=days))
    print(f"[sync_cnw] mining items from GNews: {len(items)}")
    ...
    for ev in items:
        ...
        if ticker not in idx:
            name = cng.extract_company_name(ev["raw_headline"], ev["raw_body"])
        ...
        ev_for_quality["_source_name"] = ev.get("source_name", "cnw")
    ...
    if onboarded:
        ...
        print(f"[sync_cnw] tickers.json saved ({len(tickers)} entries)")

    # 6. Backfill slugs for any new events from this source
    n_slug = 0
    for eid, hl in conn.execute(
        "SELECT event_id, raw_headline FROM events "
        "WHERE source_name='cnw' AND (slug IS NULL OR slug = '')"
    ):
    ...
    print(f"[sync_cnw] DONE  ingested={ingested}  fuzzy_dup_skip={dup_skip}  "
