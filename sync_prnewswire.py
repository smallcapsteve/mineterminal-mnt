"""sync_prnewswire.py — pull PR Newswire mining releases via Google News, dedupe, ingest.

PR Newswire's native mining-specific RSS feed is unreliable (subjectcode/9
...
PR Newswire pages fetch cleanly so we get full bodies and extract Canadian
tickers directly. New mining tickers are auto-onboarded into tickers.json.

source_name="prnewswire" — same as the per-company adapter, so the
...
from sources import prnewswire_gnews as prng  # noqa: E402
...
def auto_onboard(ticker: str, name: str | None, exchange: str | None,
    ...
    new = {
        ...
        "source": "prnewswire_gnews",
        "source_params": {},
        "discovery_confidence": "prnewswire_auto",
        "discovered_at": today,
    }
    if exchange:
...
def main(days: int = 7) -> int:
    ...
    print(f"[sync_prnewswire] start {started}  days={days}")

    tickers = load_tickers()
    idx = known_tickers_index(tickers)
    print(f"[sync_prnewswire] known tickers: {len(idx)}")

    items = list(prng.list_mining_events(days=days))
    print(f"[sync_prnewswire] mining items from GNews: {len(items)}")
    ...
    for ev in items:
        ...
        if ticker not in idx:
            name = prng.extract_company_name(ev["raw_headline"], ev["raw_body"])
        ...
        ev_for_quality["_source_name"] = ev.get("source_name", "prnewswire")
    ...
    if onboarded:
        ...
        print(f"[sync_prnewswire] tickers.json saved ({len(tickers)} entries)")

    # 6. Backfill slugs for any new events from this source
    n_slug = 0
    for eid, hl in conn.execute(
        "SELECT event_id, raw_headline FROM events "
        "WHERE source_name='prnewswire' AND (slug IS NULL OR slug = '')"
    ):
    ...
    print(f"[sync_prnewswire] DONE  ingested={ingested}  fuzzy_dup_skip={dup_skip}  "
